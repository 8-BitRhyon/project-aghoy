...
import { ShieldCheck, ShieldAlert, AlertTriangle, ChevronRight, BookOpen, Copy, Check, Phone, Globe, Share2, Building2, Mail, Lock, FileText, Smartphone, ExternalLink, Info } from 'lucide-react';
...
const ResultCard: React.FC<ResultCardProps> = ({ result, onReset }) => {
    ...
    switch (verdict) {
      case Verdict.SAFE:
        return {
          ...
          icon: <ShieldCheck className="w-8 h-8 md:w-12 md:h-12 text-green-400" />,
          label: 'STATUS: SAFE'
        };
      case Verdict.SUSPICIOUS:
        return {
          ...
          icon: <AlertTriangle className="w-8 h-8 md:w-12 md:h-12 text-yellow-400" />,
          label: 'STATUS: SUSPICIOUS'
        };
      case Verdict.HIGH_RISK:
        return {
          ...
          icon: <ShieldAlert className="w-8 h-8 md:w-12 md:h-12 text-red-500" />,
          label: 'STATUS: HIGH_RISK'
        };
    }
  };
  ...
  // Official Channels Configuration - Expanded
  ...
  return (
    <div key={result.riskScore + result.verdict} className="w-full mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 font-['VT323'] px-0 md:px-0 pb-10">
      ...
      {/* Main Card Container with chunky borders and shadow */}
      <div className={`border-[4px] md:border-[6px] ${style.borderColor} bg-slate-900 p-1 shadow-[6px_6px_0_0_rgba(0,0,0,0.8)] md:shadow-[14px_14px_0_0_rgba(0,0,0,0.8)] relative m-1 md:m-2 transition-colors duration-500`}>
        ...
        {/* Decorative corner pixels (screws) */}
        ...
        <div className="border-2 border-slate-700 p-3 md:p-6 bg-[#0f172a] relative overflow-hidden">
                ...
                <div className="flex flex-wrap gap-2 mt-2 md:mt-4">
                    ...
                    {result.verdict !== Verdict.SAFE && (
                        <button 
                            ...
                            className="flex items-center gap-2 px-2 py-0.5 md:px-3 md:py-1 bg-pink-700 hover:bg-pink-600 text-white border-b-4 border-r-4 border-pink-900 active:border-0 active:translate-y-1 transition-all text-[10px] md:text-xs font-['Press_Start_2P']"
                        >
          ...
          <div className="flex flex-col lg:grid lg:grid-cols-12 gap-6 relative z-10">
            ...
            <div className="lg:col-span-4 flex flex-col gap-4">
                ...
                {result.redFlags.length > 0 && (
                    <div className="relative">
                        ...
                        {/* Interactive Tooltip Area - FIXED HEIGHT */}
                        <div className="h-32 mb-2 border border-slate-700 bg-slate-900/50 p-2 overflow-y-auto custom-scrollbar">
                             {hoveredFlag ? (
                                <div className="text-xs md:text-sm text-cyan-300 font-mono animate-fade-in">
                                    <span className="font-bold text-white block mb-1 uppercase border-b border-cyan-500/30 pb-1">
                                      INFO: {hoveredFlag}
                                    </span>
                                    {getFlagDefinition(hoveredFlag)}
                                </div>
                             ) : (
                                <div className="h-full flex items-center justify-center text-xs text-slate-600 font-mono italic text-center">
                                    [ HOVER FLAGS FOR INTEL ]
                                </div>
                             )}
                        </div>
                ...
                {/* PNP Report Button (Only for High Risk) - FIXED LAYOUT */}
                {result.verdict === Verdict.HIGH_RISK && (
                    <div className="w-full bg-red-700 text-white border-b-4 md:border-b-8 border-r-4 md:border-r-8 border-red-900 flex flex-col p-3 md:p-4 gap-3 relative overflow-hidden group">
                        ...
                        <button 
                            ...
                            className={`w-full flex items-center justify-center gap-2 text-xs md:text-sm font-bold bg-red-900 hover:bg-red-800 px-3 py-2 border-2 ${copied ? 'border-green-400 text-green-400' : 'border-white text-white'} font-['Press_Start_2P'] transition-all active:scale-95 relative z-10`}
                        >
                ...
                {/* Official Channels - EXPANDED VERTICAL LAYOUT */}
                {(result.verdict === Verdict.SUSPICIOUS || result.verdict === Verdict.HIGH_RISK) && (
                                ...
                                >
                                    ...
                                    <div className="min-w-0 flex-1 flex flex-col">
                                        <div className="flex justify-between items-baseline">
                                            <span className={`text-sm md:text-base font-bold text-slate-200 ${channel.groupHoverText} font-['Press_Start_2P']`}>
                                                {channel.label}
                                            </span>
                                            <span className="text-[10px] md:text-xs text-slate-500 uppercase tracking-tighter hidden sm:inline">
                                                {channel.desc}
                                            </span>
                                        </div>
            ...
            {/* RIGHT COLUMN (Detailed Analysis & Education) */}
            <div className="lg:col-span-8 flex flex-col gap-6">
                
                {/* Detailed Analysis - Terminal Box */}
                <div className="bg-black p-3 md:p-6 border-[3px] border-green-900/50 shadow-inner h-fit">
                    <div className="flex items-center justify-between border-b border-green-900/50 pb-2 mb-4">
                        <div className="bg-green-900 text-green-100 px-2 py-0.5 text-[10px] md:text-xs font-bold font-['Press_Start_2P']">
                            SYS_LOG
                        </div>
                    ...
                    <p className="text-green-400 leading-relaxed text-sm md:text-xl whitespace-pre-line font-mono break-words">
                        {">"} {result.analysis}
                        <span className="inline-block w-2 h-4 md:w-3 md:h-6 bg-green-400 ml-1 animate-pulse align-middle"></span>
                    </p>
                </div>
                ...
                {result.verdict !== Verdict.SAFE && (
                    <div className="border-t-4 border-slate-800 pt-4">
                        <button 
                            ...
                            className="flex items-center justify-between w-full text-left text-cyan-400 hover:text-cyan-300 transition-colors group mb-4"
                        >
                        ...
                        {expandedEducation && (
                            ...
                            <div className="bg-blue-950/50 border-l-4 md:border-l-8 border-blue-500 p-3 md:p-6">
                                <div className="flex flex-col md:flex-row items-start gap-3">
                                <div className="p-2 md:p-3 bg-blue-600 text-white shrink-0 border-2 border-white shadow-[4px_4px_0_0_rgba(0,0,0,0.5)] hidden md:block">
                                ...
                                <div>
                                    ...
                                    <p className="text-blue-100 text-sm md:text-xl leading-relaxed italic border-l-2 border-blue-500/30 pl-3 md:pl-4">
                ...
                {/* Emergency Response Guide (For victims) */}
                {(result.verdict === Verdict.SUSPICIOUS || result.verdict === Verdict.HIGH_RISK) && (
                    <div className="border-[4px] border-red-900 bg-red-950/30 p-3 md:p-6 relative overflow-hidden">
                        ...
                        <h4 className="text-red-400 text-[10px] md:text-sm font-['Press_Start_2P'] mb-4 flex items-center gap-2 uppercase tracking-wide">
                            <AlertTriangle className="w-3 h-3 md:w-4 md:h-4 animate-bounce" /> 
                            VICTIM_ASSISTANCE PROTOCOL
                        </h4>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 font-['VT323'] text-sm md:text-xl text-slate-300">
                            ...
                            <div className="flex md:flex-col gap-3 md:gap-2">
                                <div className="flex-shrink-0 w-8 h-8 md:w-12 md:h-12 bg-red-900/50 border-2 border-red-500 flex items-center justify-center text-red-400 font-bold text-base md:text-2xl font-['Press_Start_2P'] shadow-[2px_2px_0_0_rgba(220,38,38,0.5)]">
                                    1
                                </div>
                                <div>
                                    <h5 className="text-red-300 font-bold text-base md:text-2xl mb-1 flex items-center gap-2">
                                    <Lock className="w-4 h-4 md:w-6 md:h-6" /> FREEZE FUNDS
                                    </h5>
                                    <p className="leading-tight mb-1 text-sm md:text-lg">
                                    Contact support to <strong>LOCK</strong> accounts.
                                    </p>
                            ...
                            {/* Step 2 */}
                            <div className="flex md:flex-col gap-3 md:gap-2">
                                <div className="flex-shrink-0 w-8 h-8 md:w-12 md:h-12 bg-orange-900/50 border-2 border-orange-500 flex items-center justify-center text-orange-400 font-bold text-base md:text-2xl font-['Press_Start_2P'] shadow-[2px_2px_0_0_rgba(234,88,12,0.5)]">
                                    2
                                </div>
                                <div>
                                    <h5 className="text-orange-300 font-bold text-base md:text-2xl mb-1 flex items-center gap-2">
                                        <FileText className="w-4 h-4 md:w-6 md:h-6" /> EVIDENCE
                                    </h5>
                                    <p className="leading-tight mb-1 text-sm md:text-lg">
                            ...
                            {/* Step 3 */}
                            <div className="flex md:flex-col gap-3 md:gap-2">
                                <div className="flex-shrink-0 w-8 h-8 md:w-12 md:h-12 bg-slate-800 border-2 border-slate-500 flex items-center justify-center text-slate-400 font-bold text-base md:text-2xl font-['Press_Start_2P'] shadow-[2px_2px_0_0_rgba(100,116,139,0.5)]">
                                    3
                                </div>
                                <div>
                                    <h5 className="text-slate-300 font-bold text-base md:text-2xl mb-1 flex items-center gap-2">
                                        <Smartphone className="w-4 h-4 md:w-6 md:h-6" /> RESET
                                    </h5>
                                    <p className="leading-tight text-sm md:text-lg">
                                    Change MPIN, passwords, and enable 2FA on all apps.
                                    </p>
                                </div>
                            </div>
                        </div>
